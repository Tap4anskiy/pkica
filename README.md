...
- Проект предназначен для обучения и дипломной демонстрации базовых механизмов PKI. Не используйте его как production CA без отдельного аудита, аппаратной или внешней защиты ключей, процедур резервного копирования и регламентов эксплуатации.
- Для Root CA и Intermediate CA используйте `--encrypt`; незашифрованные ключи допустимы только для тестов.
- Пароли к ключам Root CA и Intermediate CA вводите через интерактивный запрос. Не сохраняйте реальные пароли в shell history, README, скриптах, issue, логах или переменных окружения.
...
- Переменные `ROOT_PASS`, `INTER_PASS`, `SERVER1_PASS`, `SERVER2_PASS`, `CLIENT_PASS` в демонстрационном скрипте предназначены только для воспроизводимых тестовых запусков. Для реального безопасного сценария лучше вводить пароли интерактивно.
