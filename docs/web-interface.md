...
После настройки nginx откройте `https://pkica.local/`. Корневой URL ведёт на публичный `Trust Center`, где пользователи могут скачать trust bundle и CRL.

Администрирование доступно отдельно по `https://pkica.local/admin`. Через админскую панель можно отправлять CSR-заявки, одобрять или отклонять заявки, выпускать и отзывать сертификаты, публиковать CRL, проверять сертификаты и просматривать журнал аудита.

При первом `pkica web start` создаются случайные логин и пароль администратора. Пароль выводится только один раз; в `data/web/private/admin-auth.json` сохраняется только хеш пароля и секрет сессий.

Страница `Trust` публикует только открытые артефакты УЦ: Root CA, Intermediate CA, `ca-chain.pem` и `intermediate.crl.pem`. Перед установкой trust пользователи должны сверить SHA256-отпечатки Root CA и Intermediate CA по независимому каналу. Те же отпечатки выводит существующая команда:
